...
def mouton_3_corps(t_i, t_f, N):
    ...
    # calcul du point r(t+h/2) avec Runge-Kutta d'ordre 2
    k1_A = h*F("A", r_Ai, r_Bi, r_Ci)
    k1_B = h*F("B", r_Ai, r_Bi, r_Ci)
    k1_C = h*F("C", r_Ai, r_Bi, r_Ci)

    k2_A = h*F("A", r_Ai + 0.5*k1_A, r_Bi + 0.5*k1_B, r_Ci + 0.5*k1_C)
    k2_B = h*F("B", r_Ai + 0.5*k1_A, r_Bi + 0.5*k1_B, r_Ci + 0.5*k1_C)
    k2_C = h*F("C", r_Ai + 0.5*k1_A, r_Bi + 0.5*k1_B, r_Ci + 0.5*k1_C)

    r_A_2 = r_Ai + k2_A
    r_B_2 = r_Bi + k2_B
    r_C_2 = r_Ci + k2_C

    r_A = r_Ai + h*F("A", r_A_2, r_B_2, r_C_2)
    r_B = r_Bi + h*F("B", r_A_2, r_B_2, r_C_2)
    r_C = r_Ci + h*F("C", r_A_2, r_B_2, r_C_2)
    ...
    for i, t in enumerate(t_points):
        ...
        r_A = r_A_1 + h*F("A", r_A_2, r_B_2, r_C_2)
        r_B = r_B_1 + h*F("B", r_A_2, r_B_2, r_C_2)
        r_C = r_C_1 + h*F("C", r_A_2, r_B_2, r_C_2)
